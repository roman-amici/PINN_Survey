...
def poisson_load_raw():
    ...
    filename = f"{path}/poisson_2d_steep.npy"
    XU = np.load(filename)

    return XU


def load_poisson_flat():
...
def load_poisson_grid():
    return to_grid(poisson_load_raw())


def load_poisson_bounds():
    X, U, [x, y, u] = load_poisson_flat()
